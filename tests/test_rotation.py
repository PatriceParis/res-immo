...
def test_les_agences_jamais_visitees_passent_en_premier(depot):
    ...
    depot.joinpath("data/annonces_reel.json").write_text(json.dumps([
        {"id": "a", "agence": "Déjà vue", "revue_le": "2026-08-04"},
    ]), encoding="utf-8")
...
def test_l_agence_vue_il_y_a_le_plus_longtemps_repasse_la_premiere(depot):
    ...
    depot.joinpath("data/annonces_reel.json").write_text(json.dumps([
        {"id": "a", "agence": "Récente", "revue_le": "2026-08-04"},
        {"id": "b", "agence": "Ancienne", "revue_le": "2026-06-01"},
        {"id": "c", "agence": "Intermédiaire", "revue_le": "2026-07-15"},
    ]), encoding="utf-8")
...
def test_une_agence_sans_aucun_bien_ne_bloque_pas_la_rotation(depot):
    ...
    depot.joinpath("data/annonces_reel.json").write_text(json.dumps([
        {"id": "a", "agence": "Site sain", "revue_le": "2026-08-01"},
    ]), encoding="utf-8")
    ...
    collecteur._noter_visite("Site cassé", "2026-08-04")
...
def test_le_journal_de_visites_survit_aux_collectes_successives(depot):
    collecteur._noter_visite("Agence A", "2026-08-01")
    collecteur._noter_visite("Agence B", "2026-08-02")
    collecteur._noter_visite("Agence A", "2026-08-04")     # revue plus tard

    journal = json.loads(
        depot.joinpath("data/agences_visitees.json").read_text(encoding="utf-8"))
    assert journal == {"Agence A": "2026-08-04", "Agence B": "2026-08-02"}
