...
- `scraper/` — Scrapy poli (robots.txt, 1 req/2,5 s, UA honnête) ; spiders
  héritent de `spiders/base.py` (parsing heuristique). Pipeline → géocodage
  BAN → `preparer_annonce()` → même base SQLite.
