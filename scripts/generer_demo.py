"""Génère data/annonces_demo.json — jeu de démonstration FICTIF mais réaliste.

Les communes, coordonnées, altitudes et niveaux de prix sont plausibles ;
les biens eux-mêmes sont inventés. Ce jeu de données sert à démontrer le
score, les filtres et la carte sans dépendre d'une collecte en ligne.
...
def _choix_dpe(equipements: set) -> str:
    dpe = random.choices([d for d, _ in DPE_PONDERATION],
...
def generer_annonce(i: int) -> dict:
    nom, cp, dept, region, lat, lon, alt, densite, prix_m2, extra = random.choice(COMMUNES)
    type_bien, profil = random.choice(list(TYPES.items()))

    surf = random.randrange(*profil["surf"], 5)
    terrain = random.randrange(*profil["terrain"], 50)
    pieces = max(2, round(surf / 32) + random.randint(-1, 1))

    equipements = {e for e, proba in profil["equip"].items() if random.random() < proba}
    etat, coef_etat, phrase_etat = random.choice(ETATS)
    dpe = _choix_dpe(equipements)

    prix = int(surf * prix_m2 * profil["facteur"] * coef_etat
               * random.uniform(0.85, 1.15) // 1000 * 1000)

    # Risques : ceux de la commune, ajustés par le type de bien.
    risques = {}
    if extra.get("inondation") and (type_bien == "moulin" or random.random() < 0.5):
        risques["inondation"] = True
    if type_bien == "moulin" and random.random() < 0.7:
        risques["inondation"] = True
    if extra.get("argile"):
        risques["argile"] = extra["argile"]
    if extra.get("feu_foret") and random.random() < 0.7:
        risques["feu_foret"] = True
    if extra.get("seveso_km") and random.random() < 0.6:
        risques["seveso_km"] = extra["seveso_km"] + round(random.uniform(-1, 3), 1)
    risques["source"] = "démo"
    ...
    dep_surface = random.randrange(40, 160, 10)
    phrases = [
    ...
    for cle in ("cave", "puits", "eau", "pluie", "bois", "solaire",
        ...
        if cle in equipements:
            phrases.append(random.choice(PHRASES[cle]).format(dep=dep_surface))
    if phrase_etat:
    ...
    return {
        ...
        "lon": round(lon + random.uniform(-0.02, 0.02), 4),
        "altitude": alt + random.randint(-15, 25),
...
def main() -> None:
    annonces = [generer_annonce(i) for i in range(1, 73)] + VITRINES
